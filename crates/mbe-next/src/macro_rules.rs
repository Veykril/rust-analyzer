...
use crate as mbe;
use crate::diagnostics::{annotate_doc_comment, parse_failure_msg};
use crate::macro_check;
use crate::macro_parser::{Error, ErrorReported, Failure, Success, TtParser};
use crate::macro_parser::{MatcherLoc, NamedMatch::*};
...
impl<'a> ParserAnyMacro<'a> {
    pub(crate) fn make(mut self: Box<ParserAnyMacro<'a>>, kind: AstFragmentKind) -> AstFragment {
        ...
        // but `m!()` is allowed in expression positions (cf. issue #33953).
    ...
    match rhs {
        mbe::TokenTree::Delimited(.., d) => {
            ...
            if has_compile_error {
                ...
                d.tts.iter().any(has_compile_error_macro)
            }
        }
        _ => false,
    }
