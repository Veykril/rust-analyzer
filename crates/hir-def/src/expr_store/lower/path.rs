...
pub(super) fn lower_path(collector: &mut ExprCollector<'_>, mut path: ast::Path) -> Option<Path> {
    ...
    loop {
        ...
        match segment.kind()? {
            ast::PathSegmentKind::Name(name_ref) => {
                ...
                let args = segment
                    .generic_arg_list()
                    .and_then(|it| collector.lower_generic_args(it))
                    .or_else(|| {
            ...
            ast::PathSegmentKind::Type { type_ref, trait_ref } => {
                ...
                let self_type = collector.lower_type_ref(type_ref?);
                ...
                match trait_ref {
                    ...
                    Some(trait_ref) => {
                        let path = collector.lower_path(trait_ref.path()?)?;
