...
use crate::{
    AdtId, BlockId, BlockLoc, ConstBlockLoc, DefWithBodyId, GenericDefId, ItemTreeLoc, MacroId,
    ModuleDefId, ModuleId, StructId, UnresolvedMacro,
    attr::Attrs,
...
) -> (Arc<ExpressionStore>, Arc<GenericParams>) {
    let mut expr_collector = ExprCollector::new(db, module, type_ref.file_id);
    let type_ref = expr_collector.lower_type_ref(type_ref.value);
    (expr_collector.store.finish(), expr_collector.source_map, type_ref)
}
...
pub(crate) fn lower_impl(
    db: &dyn DefDatabase,
    module: ModuleId,
    impl_: InFile<ast::Impl>,
) -> (ExpressionStore, ExpressionStoreSourceMap, TypeRefId, Option<PathId>, Arc<GenericParams>) {
    let mut expr_collector = ExprCollector::new(db, module, impl_.file_id);
    let self_ty = expr_collector.lower_type_ref_opt(impl_.value.self_ty());
    let trait_ = impl_.value.trait_().and_then(|it| match &it {
        ast::Type::PathType(path_type) => Some(
            expr_collector
                .alloc_path(expr_collector.lower_path_type(&path_type)?, AstPtr::new(&it)),
        ),
        _ => None,
    });
    (expr_collector.store.finish(), expr_collector.source_map, self_ty)
}

pub(crate) fn lower_type_alias(
    db: &dyn DefDatabase,
    module: ModuleId,
    impl_: InFile<ast::TypeAlias>,
) -> (ExpressionStore, Arc<GenericParams>, Box<[TypeBound]>) {
    todo!()
}
pub struct ExprCollector<'a> {
...
impl ExprCollector<'_> {
    ...
    pub fn lower_type_ref(&mut self, node: ast::Type) -> TypeRefId {
        let ty = match &node {
            ast::Type::ParenType(inner) => return self.lower_type_ref_opt(inner.ty()),
            ast::Type::TupleType(inner) => TypeRef::Tuple(EmptyOptimizedThinVec::from_iter(
                Vec::from_iter(inner.fields().map(|it| self.lower_type_ref(it))),
            )),
            ...
            ast::Type::PathType(inner) => {
                ...
                inner
                    .path()
                    .and_then(|it| self.lower_path(it))
                    .map(TypeRef::Path)
                    .unwrap_or(TypeRef::Error)
            }
            ast::Type::PtrType(inner) => {
                let inner_ty = self.lower_type_ref_opt(inner.ty());
                let mutability = Mutability::from_mutable(inner.mut_token().is_some());
                TypeRef::RawPtr(inner_ty, mutability)
            }
            ast::Type::ArrayType(inner) => {
                let len = self.const_ref_from_const_arg(inner.const_arg());
                TypeRef::Array(Box::new(ArrayType { ty: self.lower_type_ref_opt(inner.ty()), len }))
            }
            ast::Type::SliceType(inner) => TypeRef::Slice(self.lower_type_ref_opt(inner.ty())),
            ...
            ast::Type::FnPtrType(inner) => {
                ...
                let mut params = if let Some(pl) = inner.param_list() {
                    ...
                    pl.params()
                        .map(|it| {
                            let type_ref = self.lower_type_ref_opt(it.ty());
            ...
            ast::Type::ForType(inner) => return self.lower_type_ref_opt(inner.ty()),
            ast::Type::ImplTraitType(inner) => {
                if self.outer_impl_trait {
                    ...
                    self.with_outer_impl_trait_scope(true, |this| {
                        let type_bounds = this.type_bounds_from_ast(inner.type_bound_list());
                        TypeRef::ImplTrait(type_bounds)
                    })
                }
            }
            ast::Type::DynTraitType(inner) => {
                TypeRef::DynTrait(self.type_bounds_from_ast(inner.type_bound_list()))
            }
            ast::Type::MacroType(mt) => match mt.macro_call() {
                Some(mcall) => {
                    ...
                    let id = self.collect_macro_call(mcall, macro_ptr, true, |this, expansion| {
                        this.lower_type_ref_opt(expansion)
                    });
    ...
    fn lower_type_ref_opt(&mut self, node: Option<ast::Type>) -> TypeRefId {
        match node {
            Some(node) => self.lower_type_ref(node),
    ...
    pub fn lower_path(&mut self, ast: ast::Path) -> Option<Path> {
        super::lower::path::lower_path(self, ast)
    }
    ...
    pub fn lower_generic_args_from_fn_path(
        ...
        for param in params.type_args() {
            let type_ref = self.lower_type_ref_opt(param.ty());
            param_types.push(type_ref);
        }
        let args = Box::new([GenericArg::Type(self.alloc_type_ref_desugared(TypeRef::Tuple(
            EmptyOptimizedThinVec::from_iter(param_types),
        )))]);
        let bindings = if let Some(ret_type) = ret_type {
            let type_ref = self.lower_type_ref_opt(ret_type.ty());
    ...
    pub(super) fn lower_generic_args(&mut self, node: ast::GenericArgList) -> Option<GenericArgs> {
        let mut args = Vec::new();
        let mut bindings = Vec::new();
        for generic_arg in node.generic_args() {
            match generic_arg {
                ast::GenericArg::TypeArg(type_arg) => {
                    let type_ref = self.lower_type_ref_opt(type_arg.ty());
                    ...
                    if let Some(name_ref) = assoc_type_arg.name_ref() {
                        ...
                        self.with_outer_impl_trait_scope(false, |this| {
                            ...
                            let args = assoc_type_arg
                                .generic_arg_list()
                                .and_then(|args| this.lower_generic_args(args))
                                .or_else(|| {
                                    assoc_type_arg
                                        .return_type_syntax()
                                        .map(|_| GenericArgs::return_type_notation())
                                });
                            let type_ref = assoc_type_arg.ty().map(|it| this.lower_type_ref(it));
                            let bounds = if let Some(l) = assoc_type_arg.type_bound_list() {
                                l.bounds().map(|it| this.lower_type_bound(it)).collect()
                            } else {
                                Box::default()
                            };
                ...
                ast::GenericArg::ConstArg(arg) => {
                    let arg = self.const_ref_from_const_arg(Some(arg));
    ...
    fn type_bounds_from_ast(
        ...
        if let Some(type_bounds) = type_bounds_opt {
            ThinVec::from_iter(Vec::from_iter(
                type_bounds.bounds().map(|it| self.lower_type_bound(it)),
            ))
        } else {
            ThinVec::from_iter([])
        }
    ...
    fn lower_path_type(&mut self, path_type: &ast::PathType) -> Option<Path> {
        let path = self.lower_path(path_type.path()?)?;
        Some(path)
    }

    fn lower_type_bound(&mut self, node: ast::TypeBound) -> TypeBound {
        match node.kind() {
            ast::TypeBoundKind::PathType(path_type) => {
                ...
                self.lower_path_type(&path_type)
                    .map(|p| {
            ...
            ast::TypeBoundKind::ForType(for_type) => {
                ...
                let path = for_type.ty().and_then(|ty| match &ty {
                    ast::Type::PathType(path_type) => {
                        self.lower_path_type(path_type).map(|p| (p, ty))
                    }
                    _ => None,
                });
                match path {
    ...
    fn const_ref_from_const_arg(&mut self, arg: Option<ast::ConstArg>) -> ConstRef {
        if let Some(arg) = arg {
            if let Some(expr) = arg.expr() {
                return ConstRef { expr: Some(self.collect_expr(expr)) };
            }
        }
        ConstRef { expr: None }
    }
    ...
    fn maybe_collect_expr(&mut self, expr: ast::Expr) -> Option<ExprId> {
        ...
        Some(match expr {
                ...
                Some(ast::BlockModifier::Const(_)) => {
                    self.with_label_rib(RibKind::Constant, |this| {
                        this.with_awaitable_block(Awaitable::No("constant block"), |this| {
                            ...
                            let _inner_expr = this.collect_block(e);
                            // FIXME:
            ...
            ast::Expr::MethodCallExpr(e) => {
                ...
                let generic_args =
                    e.generic_arg_list().and_then(|it| self.lower_generic_args(it)).map(Box::new);
                self.alloc_expr(
            ...
            ast::Expr::CastExpr(e) => {
                let expr = self.collect_expr_opt(e.expr());
                let type_ref = self.lower_type_ref_opt(e.ty());
            ...
            ast::Expr::ClosureExpr(e) => self.with_label_rib(RibKind::Closure, |this| {
                ...
                if let Some(pl) = e.param_list() {
                    ...
                    for param in pl.params() {
                        ...
                        let type_ref = param.ty().map(|it| this.lower_type_ref(it));
                        args.push(pat);
                        arg_types.push(type_ref);
                    }
                }
                let ret_type = e.ret_type().and_then(|r| r.ty()).map(|it| this.lower_type_ref(it));
            ...
            ast::Expr::OffsetOfExpr(e) => {
                let container = self.lower_type_ref_opt(e.ty());
    ...
    fn collect_stmt(&mut self, statements: &mut Vec<Statement>, s: ast::Stmt) {
        match s {
            ast::Stmt::LetStmt(stmt) => {
                ...
                let type_ref = stmt.ty().map(|it| self.lower_type_ref(it));
