...
/// will consume).
pub fn syntax_node_to_token_tree<Anchor: Copy>(
    node: &SyntaxNode,
    anchor: Anchor,
    anchor_offset: TextSize,
) -> tt::Subtree<SpanData<Anchor>>
where
    SpanData<Anchor>: Span,
{
    assert!(anchor_offset <= node.text_range().start());
    let mut c = Converter::new(node, anchor_offset, anchor);
    ...
    SpanAnchor: Copy,
{
    ...
    loop {
        ...
        if kind == COMMENT {
            if let Some(tokens) = conv.convert_doc_comment(&token, SpanData { range, anchor }) {
        ...
        let tt = if kind.is_punct() && kind != UNDERSCORE {
            ...
            if let Some(expected) = expected {
                if kind == expected {
                    if let Some(mut subtree) = stack.pop() {
                        subtree.delimiter.close = SpanData { range, anchor };
            ...
            tt::Leaf::from(tt::Punct { char, spacing, span: SpanData { range, anchor } }).into()
        } else {
            macro_rules! make_leaf {
                ($i:ident) => {
                    tt::$i { span: SpanData { range, anchor }, text: token.to_text(conv) }.into()
                };
            }
...
impl<SpanAnchor> Converter<SpanAnchor> {
    fn new(
        node: &SyntaxNode,
        anchor_offset: TextSize,
        file_id: SpanAnchor,
    ) -> Converter<SpanAnchor> {
        let range = node.text_range();
        let mut preorder = node.preorder_with_tokens();
        let first = Self::next_token(&mut preorder);
        Converter {
            current: first,
            preorder,
            range,
    ...
    fn next_token(preorder: &mut PreorderWithTokens) -> Option<SyntaxToken> {
        while let Some(ev) = preorder.next() {
            if let WalkEvent::Enter(SyntaxElement::Token(t)) = ev {
                return Some(t);
            }
        }
        None
...
impl<SpanAnchor> SrcToken<Converter<SpanAnchor>> for SynToken {
    fn kind(&self, ctx: &Converter<SpanAnchor>) -> SyntaxKind {
        match self {
            SynToken::Ordinary(token) => token.kind(),
            SynToken::Punct(..) => SyntaxKind::from_char(self.to_char(ctx).unwrap()).unwrap(),
        }
    }
    fn to_char(&self, _ctx: &Converter<SpanAnchor>) -> Option<char> {
        match self {
            SynToken::Ordinary(_) => None,
            SynToken::Punct(it, i) => it.text().chars().nth(*i),
        }
    }
    fn to_text(&self, _ctx: &Converter<SpanAnchor>) -> SmolStr {
        match self {
...
impl<SpanAnchor: Copy> TokenConverter<SpanAnchor> for Converter<SpanAnchor>
where
    SpanData<SpanAnchor>: Span,
{
    type Token = SynToken;
    ...
    fn bump(&mut self) -> Option<(Self::Token, TextRange)> {
        ...
        self.current = Self::next_token(&mut self.preorder);
        let token = if curr.kind().is_punct() {
            self.punct_offset = Some((curr.clone(), 0.into()));
            let range = curr.text_range();
            let range = TextRange::at(range.start(), TextSize::of('.'));
            (SynToken::Punct(curr, 0 as usize), range - self.offset)
        } else {
            self.punct_offset = None;
            let range = curr.text_range();
            (SynToken::Ordinary(curr), range - self.offset)
        };
