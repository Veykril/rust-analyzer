...
mod tests {
    ...
    fn test_quote_hash_token_tree() {
        ...
        let t = format!("{quoted:?}");
        assert_eq!(t, "SUBTREE $$ SpanData { range: 0..0, file_id: FileId(FileId(0)) } SpanData { range: 0..0, file_id: FileId(FileId(0)) }\n  IDENT   hello SpanData { range: 0..0, file_id: FileId(FileId(0)) }");
    }
