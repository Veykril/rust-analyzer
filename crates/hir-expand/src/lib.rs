...
pub type TokenMap = ::mbe::TokenMap<tt::SpanData>;
...
impl ExpansionInfo {
    ...
    pub fn map_token_up(
        ...
        self.exp_map.token_by_range(token.value.text_range()).and_then(|span| {
            InFile::new(
                span.anchor.file_id,
                db.parse_or_expand(span.anchor.file_id).covering_element(span.range).into_token(),
            )
            .transpose()
        })
    }
...
fn ascend_call_token(
    ...
    while let Some(info) = mapping.file_id.expansion_info(db) {
        mapping = info.map_token_up(db, mapping.as_ref())?;
    }
    None
}
