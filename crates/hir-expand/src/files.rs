...
use crate::{
    EditionedFileId, ExpansionInfo, HirFileId, MacroCallId, MacroExpander,
    db::{self, ExpandDatabase},
    map_node_range_up, map_node_range_up_rooted,
...
impl<SN: Borrow<SyntaxNode>> InFile<SN> {
    pub fn parent_ancestors_with_macros(
        ...
        let succ = move |node: &InFile<SyntaxNode>| match node.value.parent() {
            ...
            None => db
                .macro_call_kind(node.file_id.macro_file()?)
                .to_node_item(db)
                .syntax()
                .cloned()
    ...
    pub fn original_syntax_node_rooted(
        ...
        let file_id = match self.file_id {
            ...
            HirFileId::MacroFile(m)
                if matches!(
                    db.macro_call_expander(m),
                    MacroExpander::BuiltInAttr(_)
                        | MacroExpander::ProcMacro(_, ProcMacroKind::Attr)
                ) =>
            {
                m
            }
...
impl InMacroFile<SyntaxToken> {
    pub fn upmap_once(
        ...
        ExpansionInfo::new(db, self.file_id).map_range_up_once(db, self.value.text_range())
    }
}
    ...
    pub fn original_file_range(self, db: &dyn db::ExpandDatabase) -> FileRange {
        match self.file_id {
            ...
            HirFileId::MacroFile(mac_file) => {
                ...
                let loc = db.macro_call_kind(mac_file);
                loc.original_call_range(db)
            }
        }
    }
            ...
            HirFileId::MacroFile(mac_file) => {
                match map_node_range_up(db, &db.expansion_span_map(mac_file), self.value) {
                    ...
                    None => {
                        ...
                        (
                            loc.original_call_range(db),
                            SyntaxContext::root(db.macro_call_def_crate(mac_file).data(db).edition),
                        )
                    }
                }
            }
        ...
        match self.file_id {
            ...
            HirFileId::MacroFile(mac_file) => {
                match map_node_range_up_rooted(db, &db.expansion_span_map(mac_file), self.value) {
                    ...
                    _ => {
                        let loc = db.macro_call_kind(mac_file);
                        loc.original_call_range_with_body(db)
                    }
                }
            }
        }
...
impl<N: AstNode> InFile<N> {
    pub fn original_ast_node_rooted(self, db: &dyn db::ExpandDatabase) -> Option<InRealFile<N>> {
        ...
        if !matches!(
            db.macro_call_expander(file_id),
            MacroExpander::BuiltInAttr(_) | MacroExpander::ProcMacro(_, ProcMacroKind::Attr)
        ) {
            return None;
        }
