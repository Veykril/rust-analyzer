    ...
    let nav_targets = match_ast! {
        match parent {
            ast::NameRef(name_ref) => {
                reference_definition(&sema, Either::Left(&name_ref)).to_vec()
            },
            ast::Name(name) => {
            ...
            _ => {
                if let Some(name_class) = NameClass::classify_lifetime(&sema, &token) {
                    let def = name_class.referenced_or_defined(sema.db);
                    let nav = def.try_to_nav(sema.db)?;
                    vec![nav]
                } else {
                    reference_definition(&sema, Either::Right(&token)).to_vec()
                }
            },
        }
    };
...
fn pick_best(tokens: TokenAtOffset<SyntaxToken>) -> Option<SyntaxToken> {
    ...
    fn priority(n: &SyntaxToken) -> usize {
        match n.kind() {
            IDENT | INT_NUMBER | T![lifetime] | T![self] => 2,
...
pub(crate) fn reference_definition(
    ...
    match name_ref {
        ...
        Either::Right(_) => ReferenceResult::Approximate(vec![]),
    }
}
