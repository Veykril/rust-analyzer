...
pub(crate) fn prepare_rename(
    ...
    let range = match &find_namelike(&sema, &syntax, position)
...
fn find_definition(
    ...
    let def = match find_namelike(sema, syntax, position)? {
...
fn rename_mod(
    ...
    let usages = def.usages(sema).set_scope(None).all();
