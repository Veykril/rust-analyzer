...
impl FlatTree {
    pub fn new(
        ...
        FlatTree {
            subtree: write_vec(w.subtree, SubtreeRepr::write_with_close_span),
            literal: write_vec(w.literal, LiteralRepr::write_with_kind),
            punct: write_vec(w.punct, PunctRepr::write),
            ident: write_vec(w.ident, IdentRepr::write_with_rawness),
    ...
    pub fn to_subtree_resolved(
        ...
        Reader {
            subtree: read_vec(self.subtree, SubtreeRepr::read_with_close_span),
            literal: read_vec(self.literal, LiteralRepr::read_with_kind),
            punct: read_vec(self.punct, PunctRepr::read),
            ident: read_vec(self.ident, IdentRepr::read_with_rawness),
            ...
            let idx_tag = match child {
                ...
                tt::iter::TtElement::Leaf(leaf) => match leaf {
                    ...
                    tt::Leaf::Ident(ident) => {
                        ...
                        let text = self.intern(ident.sym.as_str());
                        ...
                        s.push(
                            tt::Leaf::Literal(tt::Literal {
                                ...
                                suffix: if repr.suffix != !0 {
                                    Some(Symbol::intern(self.text[repr.suffix as usize].as_str()))
                                } else {
                                    None
                                },
                            })
