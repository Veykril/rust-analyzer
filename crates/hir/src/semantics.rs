...
impl<'db> SemanticsImpl<'db> {
    ...
    pub fn speculative_expand_macro_call(
        ...
        let bs = analyzer.body_source_map()?;
        let macro_file = bs.expansion(macro_call)?;
    ...
    pub fn resolve_type(&self, ty: &ast::Type) -> Option<Type> {
        ...
        let ty =
            analyze.body_source_map()?.store.types.node_type(InFile::new(analyze.file_id, ty))?;
        let ty = hir_ty::TyLoweringContext::new(
            self.db,
            &analyze.resolver,
            &analyze.body()?.store,
            analyze.resolver.generic_def()?,
        )
        .lower_ty(ty);
