        ...
        for def in self.declarations(db) {
            match def {
                ...
                ModuleDef::Adt(adt) => {
                    match adt {
                        Adt::Struct(s) => {
                            let source_maps = db.struct_signature_with_source_map(s.id).1;
                            let source_maps = db.variant_fields_with_source_map(s.id.into()).1;
                            push_ty_diagnostics(
                                db,
                                acc,
                            ...
                            // fixme: source map diagnostics
                        }
                        Adt::Union(u) => {
                            let source_maps = db.union_signature_with_source_map(u.id).1;
                            let source_maps = db.variant_fields_with_source_map(u.id.into()).1;
                            push_ty_diagnostics(
                                db,
                                acc,
                        ...
                        Adt::Enum(e) => {
                            let source_maps = db.enum_signature_with_source_map(e.id).1;
                            for v in e.variants(db) {
                                let source_maps = db.variant_fields_with_source_map(v.id.into()).1;
                                push_ty_diagnostics(
                                    db,
                                    acc,
                ...
                ModuleDef::TypeAlias(type_alias) => {
                    let tree_source_maps = db.type_alias_signature_with_source_map(type_alias.id).1;
                    push_ty_diagnostics(
                        db,
                        acc,
    ...
    pub fn diagnostics(
        ...
        let sig_source_map = match self {
            ...
            DefWithBody::Variant(variant) => {
                ...
                db.enum_signature_with_source_map(enum_id.into()).1
            }
        };
