...
pub enum TokenTree<Span> {
    ...
    Delimited(DelimSpan<Span>, DelimSpacing, Delimiter, Subtree),
}
...
impl DelimSpacing {
    pub fn alone() -> Self {
        Self { open: Spacing::Alone, close: Spacing::Alone }
    }
}
...
impl fmt::Display for TokenKind {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            TokenKind::BinOp(BinOpToken::Plus) => f.write_str("+"),
            TokenKind::BinOp(BinOpToken::Minus) => f.write_str("-"),
            TokenKind::BinOp(BinOpToken::Star) => f.write_str("*"),
            TokenKind::BinOp(BinOpToken::Slash) => f.write_str("/"),
            TokenKind::BinOp(BinOpToken::Percent) => f.write_str("%"),
            TokenKind::BinOp(BinOpToken::Caret) => f.write_str("^"),
            TokenKind::BinOp(BinOpToken::And) => f.write_str("&"),
            TokenKind::BinOp(BinOpToken::Or) => f.write_str("|"),
            TokenKind::BinOp(BinOpToken::Shl) => f.write_str("<<"),
            TokenKind::BinOp(BinOpToken::Shr) => f.write_str(">>"),
            TokenKind::BinOpEq(BinOpToken::Plus) => f.write_str("+="),
            TokenKind::BinOpEq(BinOpToken::Minus) => f.write_str("-="),
            TokenKind::BinOpEq(BinOpToken::Star) => f.write_str("*="),
            TokenKind::BinOpEq(BinOpToken::Slash) => f.write_str("/="),
            TokenKind::BinOpEq(BinOpToken::Percent) => f.write_str("%="),
            TokenKind::BinOpEq(BinOpToken::Caret) => f.write_str("^="),
            TokenKind::BinOpEq(BinOpToken::And) => f.write_str("&="),
            TokenKind::BinOpEq(BinOpToken::Or) => f.write_str("|="),
            TokenKind::BinOpEq(BinOpToken::Shl) => f.write_str("<<="),
            TokenKind::BinOpEq(BinOpToken::Shr) => f.write_str(">>="),
            ...
            TokenKind::Pound => f.write_str("#"),
            TokenKind::Dollar => f.write_str("$"),
            TokenKind::Question => f.write_str("?"),
            TokenKind::SingleQuote => f.write_str("'"),
            TokenKind::OpenDelim(Delimiter::Parenthesis) => f.write_str("("),
            TokenKind::OpenDelim(Delimiter::Brace) => f.write_str("{"),
            TokenKind::OpenDelim(Delimiter::Bracket) => f.write_str("["),
            TokenKind::OpenDelim(Delimiter::Invisible) => f.write_str("∅"),
            TokenKind::CloseDelim(Delimiter::Parenthesis) => f.write_str(")"),
            TokenKind::CloseDelim(Delimiter::Brace) => f.write_str("}"),
            TokenKind::CloseDelim(Delimiter::Bracket) => f.write_str("]"),
            TokenKind::CloseDelim(Delimiter::Invisible) => f.write_str("∅"),
            TokenKind::Literal(lit) => write!(f, "{}", lit),
            TokenKind::Ident(ident, raw) => write!(f, "{}{}", raw.as_str(), ident),
            TokenKind::Lifetime(lifetime) => write!(f, "'{}", lifetime),
            TokenKind::DocComment(kind, style, text) => {
...
impl<S: Copy> TopSubtree<S> {
    pub fn empty(span: DelimSpan<S>) -> Self {
        Self(Box::new([TokenTree::empty(span)]))
    }

    pub fn invisible_from_leaves<const N: usize>(
        delim_span: DelimSpan<S>,
        tokens: [(Token<S>, Spacing); N],
    ) -> Self {
        let mut builder = TopSubtreeBuilder::new(delim_span, Delimiter::Invisible);
    ...
    pub fn from_token_trees(
        delim_span: DelimSpan<S>,
        delimiter: Delimiter,
        token_trees: TokenTreesView<'_, S>,
    ) -> Self {
        let mut builder = TopSubtreeBuilder::new(delim_span, delimiter);
    ...
    pub fn top_subtree_delim_span(&self) -> &DelimSpan<S> {
        let TokenTree::Delimited(span, ..) = &self.0[0] else {
            unreachable!("the first token tree is always the top subtree");
        };
        span
    }

    pub fn top_subtree_delim_span_mut(&mut self) -> &mut DelimSpan<S> {
        let TokenTree::Delimited(span, ..) = &mut self.0[0] else {
            unreachable!("the first token tree is always the top subtree");
        };
        span
    }

    pub fn top_subtree_delimiter(&self) -> Delimiter {
        let &TokenTree::Delimited(_, _, delimiter, ..) = &self.0[0] else {
            unreachable!("the first token tree is always the top subtree");
        };
        delimiter
    }
    ...
    pub fn top_subtree_delimiter_mut(&mut self) -> &mut Delimiter {
        let TokenTree::Delimited(_, _, delimiter, ..) = &mut self.0[0] else {
            unreachable!("the first token tree is always the top subtree");
        };
        delimiter
    }

    pub fn make_invis(&mut self, span: S) {
        let TokenTree::Delimited(delim_span, _, delimiter, ..) = &mut self.0[0] else {
            unreachable!("the first token tree is always the top subtree");
        };
        *delim_span = DelimSpan::from_pair(span, span);
        *delimiter = Delimiter::Invisible;
    }
...
impl<S: Copy> TopSubtreeBuilder<S> {
    pub fn new(span: DelimSpan<S>, delimiter: Delimiter) -> Self {
        let mut result = Self {
        ...
        let top_subtree =
            TokenTree::Delimited(span, DelimSpacing::alone(), delimiter, Subtree { len: 0 });
        result.token_trees.push(top_subtree);
        result
    }

    pub fn open(&mut self, delimiter_kind: Delimiter, open_span: S) {
        self.unclosed_subtree_indices.push(self.token_trees.len());
        self.token_trees.push(TokenTree::Delimited(
            DelimSpan {
                open: open_span,
    ...
    pub fn close(&mut self, close_span: S) {
        ...
        let TokenTree::Delimited(span, _, _, subtree) = &mut self.token_trees[last_unclosed_index]
        else {
            unreachable!("unclosed token tree is always a subtree");
        };
        subtree.len = subtree_len;
        span.close = close_span;
    ...
    pub fn remove_last_subtree_if_invisible(&mut self) {
        ...
        if let TokenTree::Delimited(_, _, Delimiter::Invisible, _) =
    ...
    pub fn push(&mut self, token: Token<S>, spacing: Spacing) {
        self.token_trees.push(TokenTree::Token(token, spacing));
    }

    pub fn extend(&mut self, leaves: impl IntoIterator<Item = (Token<S>, Spacing)>) {
        self.token_trees
            .extend(leaves.into_iter().map(|(token, spacing)| TokenTree::Token(token, spacing)));
    }
    ...
    pub fn expected_delimiter(&self) -> Option<Delimiter> {
        self.unclosed_subtree_indices.last().map(|&subtree_idx| {
            let TokenTree::Delimited(_, _, delimiter, _ubtree) = self.token_trees[subtree_idx]
            else {
                unreachable!("unclosed token tree is always a subtree")
            };
            delimiter
    ...
    pub fn flatten_unclosed_subtrees(&mut self) {
        for &subtree_idx in &self.unclosed_subtree_indices {
            let TokenTree::Delimited(span, spacing, delim, _) = self.token_trees[subtree_idx]
            else {
                unreachable!("unclosed token tree is always a subtree")
            };
            let delim = TokenKind::OpenDelim(delim);
            self.token_trees[subtree_idx] =
                TokenTree::Token(Token::new(delim, span.open), spacing.open);
        }
    ...
    pub fn build(mut self) -> TopSubtree<S> {
        ...
        let TokenTree::Delimited(.., top_subtree) = &mut self.token_trees[0] else {
...
impl<'a, S: Copy> TokenTreesView<'a, S> {
    pub fn new(tts: &'a [TokenTree<S>]) -> Self {
        if cfg!(debug_assertions) {
            tts.iter().enumerate().for_each(|(idx, tt)| {
                if let TokenTree::Delimited(.., tt) = &tt {
    ...
    pub fn try_into_subtree(self) -> Option<SubtreeView<'a, S>> {
        if let Some(TokenTree::Delimited(.., subtree)) = self.0.first() {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        fn token_trees_display<S>(f: &mut fmt::Formatter<'_>, iter: TtIter<'_, S>) -> fmt::Result {
            ...
            for child in iter {
                ...
                match child {
                    TtElement::Token(t, spacing) => {
                        needs_space = spacing == Spacing::Alone;
                        fmt::Display::fmt(&t.kind, f)?;
                    }
                    TtElement::Delimited(_, _, delimiter, _, subtree_iter) => {
                        ...
                        f.write_str(l)?;
                        token_trees_display(f, subtree_iter)?;
                        f.write_str(r)?;
                    }
                }
            }
...
impl<'a, S: Copy> SubtreeView<'a, S> {
    pub fn new(tts: &'a [TokenTree<S>]) -> Self {
        if cfg!(debug_assertions) {
            let TokenTree::Delimited(.., subtree) = &tts[0] else {
    ...
    pub fn top_subtree(&self) -> &'a Subtree {
        let TokenTree::Delimited(.., subtree) = &self.0[0] else {
            unreachable!("the first token tree is always the top subtree");
        };
        subtree
    }

    pub fn top_delimiter(&self) -> Delimiter {
        let TokenTree::Delimited(_, _, delim, _) = self.0[0] else {
            unreachable!("the first token tree is always the top subtree");
        };
        delim
    }
    ...
    pub fn strip_invisible(&self) -> TokenTreesView<'a, S> {
        if self.top_delimiter() == Delimiter::Invisible {
...
fn print_debug_subtree<S: fmt::Debug>(
    ...
    fmt::Debug::fmt(&delim_span.open, f)?;
    write!(f, " ")?;
    fmt::Debug::fmt(&delim_span.close, f)?;
    for child in iter {
        writeln!(f)?;
...
fn print_debug_token<S: fmt::Debug>(
    ...
    match tt {
        TtElement::Token(token, spacing) => {
            write!(
                f,
                "{}TOKEN   {} [{}] {:#?}",
                align,
                token.kind,
                if spacing == Spacing::Alone { "alone" } else { "joint" },
                token.span
            )?;
        }
        TtElement::Delimited(span, _, delim, subtree, subtree_iter) => {
            print_debug_subtree(f, delim, span, level, subtree_iter)?;
        }
    }
        ...
        fn debug_token_tree<S>(
            ...
            match tt {
                TokenTree::Token(token, _) => {
                    ...
                    match (&token.kind, last) {
                        (
                            TokenKind::Ident(_, _),
                            Some(&TokenTree::Token(Token { kind: TokenKind::Ident(_, _), .. }, _)),
                        ) => {
                            output.push(' ');
                            output.push_str(&s);
                        }
                        (_, Some(&TokenTree::Token(_, Spacing::Alone))) => {
                            output.push(' ');
                            output.push_str(&s);
                        }
                        _ => output.push_str(&s),
                ...
                TokenTree::Delimited(_, _, delim, subtree) => {
                    let delim = match delim {
                        Delimiter::Brace => ("{", "}"),
                        Delimiter::Bracket => ("[", "]"),
                        Delimiter::Parenthesis => ("(", ")"),
...
pub fn pretty<S>(mut tkns: &[TokenTree<S>]) -> String {
    fn tokentree_to_text<S>(tkn: &TokenTree<S>, tkns: &mut &[TokenTree<S>]) -> String {
        match tkn {
            TokenTree::Token(token, _) => token.kind.to_string(),
            TokenTree::Delimited(_, _, delim, subtree) => {
    ...
    while let Some((tkn, rest)) = tkns.split_first() {
        ...
        last_to_joint = false;
        if let &TokenTree::Token(_, spacing) = tkn {
