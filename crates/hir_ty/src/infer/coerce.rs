...
use crate::{autoderef, traits::Solution, Obligation, Substs, TraitRef, TyKind};
...
impl<'a> InferenceContext<'a> {
    ...
    pub(super) fn coerce(&mut self, from_ty: &TyKind, to_ty: &TyKind) -> bool {
    ...
    ///    least upper bound.
    pub(super) fn coerce_merge_branch(&mut self, ty1: &TyKind, ty2: &TyKind) -> TyKind {
        if self.coerce(ty1, ty2) {
            ty2.clone()
        } else if self.coerce(ty2, ty1) {
            ty1.clone()
        } else {
            if let (TyKind::FnDef(..), TyKind::FnDef(..)) = (ty1, ty2) {
                ...
                let ptr_ty1 = TyKind::fn_ptr(sig1);
                let ptr_ty2 = TyKind::fn_ptr(sig2);
    ...
    fn coerce_inner(&mut self, mut from_ty: TyKind, to_ty: &TyKind) -> bool {
        match (&from_ty, to_ty) {
        ...
        match (&mut from_ty, to_ty) {
            ...
            | (TyKind::Ref(Mutability::Mut, substs), &TyKind::Raw(m2, ..)) => {
                from_ty = TyKind::Raw(m2, substs.clone());
            }
    ...
    fn try_coerce_unsized(&mut self, from_ty: &TyKind, to_ty: &TyKind) -> Option<bool> {
    ...
    fn unify_autoderef_behind_ref(&mut self, from_ty: &TyKind, to_ty: &TyKind) -> bool {
