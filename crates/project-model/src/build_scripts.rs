...
impl WorkspaceBuildScripts {
    ...
    pub(crate) fn run_once<'cw>(
        config: &CargoConfig,
        workspaces: impl IntoIterator<Item = &'cw CargoWorkspace> + Clone,
        progress: &dyn Fn(String),
        ...
        let errors = Self::run_command(
            ...
            |package, cb| {
                if let Some(&(package, workspace)) = by_id.get(package) {
                    cb(&mut res[workspace].outputs[package]);
                }
            },
            progress,
        )?;
    ...
    fn run_command(
        cmd: Command,
        // ideally this would be something like, but owned trait objects aren't a thing
        // with_output_for: impl FnMut(&str, dyn FnOnce(&mut BuildScriptOutput)),
        mut with_output_for: impl FnMut(&str, &mut dyn FnMut(&mut BuildScriptOutput)),
        progress: &dyn Fn(String),
            ...
            &mut |line| {
                ...
                match message {
                    Message::BuildScriptExecuted(mut message) => {
                        with_output_for(&message.package_id.repr, &mut |data| {
                            let cfgs = {
                                let mut acc = Vec::new();
